...
import com.example.gentlepark.databinding.SpecialRvItemBinding
    ...
    inner class SpecialProductsViewHolder(private val binding:SpecialRvItemBinding):RecyclerView.ViewHolder(binding.root){
        fun bind(product: Product){
            binding.apply {
                ...
                Glide.with(itemView).load(product.images[1]).into(imageId)
                titleId.text = product.name
                priceId.text = product.price.toString()
            }
        }
    }
